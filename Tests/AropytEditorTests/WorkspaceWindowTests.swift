...
struct WorkspaceWindowTests {
    @Test func openingDirectoryInstallsSidebarToolbarAndLoadsSelectionInPlace() throws {
        ...
        #expect(windowController.window?.toolbar?.items.first?.itemIdentifier == EditorWindowController.sidebarItemID)
    ...
    @Test func toggleChangesIconStateAndShowsToast() throws {
        _ = NSApplication.shared
        let root = try makeTemporaryDirectory()
        defer { try? FileManager.default.removeItem(at: root) }
        let document = MarkdownDocument()
