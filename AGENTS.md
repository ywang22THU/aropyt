...
- Edit → Find 提供 `Cmd+F`、`Cmd+R`、`Cmd+G`、`Cmd+Shift+G`；Cmd+F 会优先使用当前编辑区的选中文本，查找栏支持实时搜索、前后跳转、首尾循环和 Esc 关闭，左侧 disclosure 可展开替换输入框。
- 源码模式直接搜索整篇 Markdown，大小写不敏感，并显示当前匹配项 / 总匹配数；扫描过程不保存全部匹配范围，避免长文档产生大量临时内存。
- 预览模式使用 `WKWebView.find` 搜索渲染后的整页可见文本，并根据 WebKit 当前选中的 DOM Range 与渲染正文计算当前匹配索引 / 总匹配数。
...
- 源码 / 预览模式切换时双向同步视窗位置。
- Cmd+F 全文查找（优先搜索编辑区当前选中文本并显示当前索引 / 文档内总数）、Cmd+R 直接打开替换、替换当前项 / 全部替换，以及 Cmd+G / Cmd+Shift+G 前后循环跳转；同时支持源码与预览模式。
...
- 2026-08-13：Cmd+F 新增选中文本自动填入，并为源码 / 预览统一显示当前索引 / 文档内总数后，Document find 7 项测试和真实 WebKit 的预览计数跳转、源码 / 预览选区接管 2 项聚焦用例通过。
